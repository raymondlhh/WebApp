...
class FavouritesService {
  static async getUserFavourites(userId) {
    try {
      ...
      const q = firebase.firestore().collection('favourites').where('userId', '==', userId);
      const snapshot = await q.get();
      const favourites = [];
      snapshot.forEach(doc => {
  ...
  static async addToFavourites(userId, itemId) {
    try {
      await firebase.firestore().collection('favourites').add({
        id: '', // Will be auto-generated by Firestore
        userId: userId, // INTEGER Foreign Key → Users(id)
  ...
  static async removeFromFavourites(favouriteId) {
    try {
      await firebase.firestore().collection('favourites').doc(favouriteId).delete();
      return true;
    } catch (error) {
  ...
  static async isItemFavourited(userId, itemId) {
    try {
      const favouritesRef = firebase.firestore().collection('favourites');
      const q = firebase.firestore().collection('favourites').where('userId', '==', userId).where('itemId', '==', itemId);
      const snapshot = await q.get();
